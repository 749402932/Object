...
import com.keren.activity.Weather;
...
public class UpdateWeather extends WebPage {
	...
	private void analyze(Document doc) {
		...
		if ("success".equals(status)) {
			...
			Elements Url=weatherDataElem.getElementsByTag("dayPictureUrl");
			...
			bundle.putString("dayPictureUrl",Url.get(0).text());
			msg.setData(bundle);//发送到主窗口的handle进行UI的更新
